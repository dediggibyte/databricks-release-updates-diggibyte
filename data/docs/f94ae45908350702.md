A standalone *streaming table* is a table registered to Unity Catalog with extra support for streaming or incremental data processing, defined outside of a Lakeflow pipeline. A pipeline is automatically created for each streaming table. You can use streaming tables for incremental data loading from Kafka and cloud object storage.
...
### Apply partial snapshot replacement with REPLACE USING flows

Beta

REPLACE USING flows are in [Beta](/aws/en/release-notes/release-types).

Use the `FLOW REPLACE USING` clause to keep a streaming table in sync with a stream of partial snapshots. On each update, a REPLACE USING flow replaces all rows that match the specified key columns and leaves all other rows unchanged. A `SEQUENCE BY` column orders the updates so the highest sequence for a key always wins, even when updates arrive out of order. For example:
...
`BY NAME` is required. It matches columns by name rather than position.

REPLACE USING behaves the same for standalone streaming tables as it does in Lakeflow pipelines. For how it works, sequencing, expectations, limitations, and examples, see [Partial snapshot replacement with REPLACE USING flows](/aws/en/ldp/flows-replace-using). The following differences apply to standalone streaming tables:

* **Define the flow in SQL.** Author the REPLACE USING flow with the inline SQL `FLOW REPLACE USING` clause on `CREATE OR REFRESH STREAMING TABLE`. The standalone `CREATE FLOW` statement is a Lakeflow pipeline construct and isn't used for standalone streaming tables.
* **Compute is managed for you.** Standalone streaming tables run on system-managed serverless pipelines and require Databricks Runtime 18.2 and above. You don't choose between classic and serverless compute.
...
Streaming tables created using SQL warehouses are automatically refreshed using a pipeline. Pipelines use the runtime in the `current` channel by default. See [Lakeflow pipelines release notes and the release upgrade process](/aws/en/release-notes/dlt/) to learn about the release process.
