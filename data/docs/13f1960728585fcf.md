...
Like other large language models (LLMs), Genie can exhibit non-deterministic behaviors. This means that you might occasionally receive different outputs when submitting the same prompt multiple times. Providing example SQL queries that Genie can learn from can help make Genie more consistent. See  [Add example SQL queries and functions](/aws/en/genie-agents/tune-quality#example-queries).
...
You select the mode when you run benchmarks, not when you add a question. Use the mode toggle at the top right of the **Benchmarks** tab to choose **Chat** or **Agent** mode for a run. The selected mode applies to all questions in that run.
...
4. (Optional) Provide a SQL query that answers the question. You can write your own query by typing in the **SQL Answer** box, including Unity Catalog SQL functions. Alternatively, click **Generate SQL** to have Genie write the SQL query for you. Use a SQL statement that accurately answers the question you entered.

   note

   This step is recommended for Chat mode runs. Only questions that include this example SQL statement can be automatically assessed for accuracy. Any questions that do not include a **SQL Answer** require manual review to be scored. If you use the **Generate SQL** button, review the statement to be sure that it's accurately answering the question.
5. (Optional) In the **Evaluation note** field, enter guidance on the correct answer or expected content. During Agent mode runs, Genie passes the evaluation note to the LLM judge. The note can reference expected content in text reports that Agent mode generates.
6. (Optional) Click **Run** to run your query and view the results.
7. When you're finished editing, click **Add benchmark**.
8. To update a question after saving, click the  pencil icon to open the **Update question** dialog.
