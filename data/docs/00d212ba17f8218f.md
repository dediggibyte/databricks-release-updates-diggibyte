...
Web search is disabled by default during the beta for all eligible workspaces. A workspace admin can turn on **Genie Code web search** from the **Previews** page. See [Manage Databricks previews](/aws/en/admin/workspace-settings/manage-previews).
...
* The workspace must be in an eligible geo (Americas or Europe), or have [cross-geography processing enabled](/aws/en/resources/databricks-geos#cross-geo-processing).
* For workspaces with the [compliance security profile](/aws/en/security/privacy/enhanced-security-compliance) enabled, web search is only supported for HIPAA. Other compliance standards are not supported.
...
* [Measure Genie Code impact](/aws/en/genie-code/impac