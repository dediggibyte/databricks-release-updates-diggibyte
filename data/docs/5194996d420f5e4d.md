...
1. For existing dashboards, open the Genie Code side panel.
...
Ask the agent to build a complete dashboard from a table or dataset.
...
Ask the agent to add individual charts and widgets to a dashboard.
...
Ask the agent to find and preview the data available to you.
...
Ask the agent to arrange pages, widgets, and layouts.
...
Ask the agent to add filters that let users interact with the dashboard.
...
* "Customize the quick options in the time range picker to only include the ones related to days."

### Data preparation

Ask the agent to create and transform the datasets behind a dashboard.
...
Ask the agent to polish an existing dashboard's appearance and details.
