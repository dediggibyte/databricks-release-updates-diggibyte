...
| --- | --- | --- | --- | --- |
| [Alert configuration](/aws/en/admin/system-tables/alerts#alerts) (Public Preview) | A slow-changing dimension table that captures the configuration of every alert over time.  **Table path**: `system.alert.alerts` | No | 365 days | Regional |
| [Alert evaluation history](/aws/en/admin/system-tables/alerts#evaluation-history) (Public Preview) | Records one row for each alert evaluation, including the evaluated state and notification delivery status.  **Table path**: `system.alert.alert_evaluation_history` | No | 365 days | Regional |
...
| [Governed tags](/aws/en/admin/system-tables/governed-tags) (Beta) | Contains versioned records for all governed tag definitions in your account, including customer-created and system-generated tags.  **Table path**: `system.tags.governed_tags` | No | 365 days | Global |
...
| [Unity Gateway usage](/aws/en/ai-gateway/usage-tracking) (Public Preview) | Captures request and response details for Unity Gateway model services including token usage, latency, performance metrics, and routing information.  **Table path**: `system.ai_gateway.usage` | Yes | 365 days | Regional |
| [Unity Gateway external model spend](/aws/en/ai-gateway/cost-observability#external-models) (Beta) | Records estimated USD spend for requests routed to external models through model provider services.  **Table path**: `system.ai_gateway.external_model_spend` | No | 365 days | Regional |
...
| [Pipeline events](/aws/en/admin/system-tables/pipeline-events) (Beta) | Records event log entries for Lakeflow pipelines in your region: lifecycle transitions, flow progress, data quality metrics, and errors.  **Table path**: `system.lakeflow_pipeline_events_preview.pipeline_events` | Yes | 365 days | Regional |
...
| [Automatic upgrades](/aws/en/admin/system-tables/automatic-upgrades) (Public Preview) | Tracks the history of feature enablement operations performed by automatic upgrades on Unity Catalog managed tables.  **Table path**: `system.storage.table_auto_upgrade_operations_history` | No | 180 days | Regional |
...
| [Replication](/aws/en/admin/system-tables/replication) | Tracks replication status for Databricks managed disaster recovery (DR).  **Table path**: `system.replication.states` | Yes | 365 days | Global |
