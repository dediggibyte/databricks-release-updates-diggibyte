...
* [Foreign Delta tables](#foreign-delta)
...
* [AI models](#models)
* [Genie Agents](/aws/en/opensharing/share-genie-agent)
...
| Add the following to a share:   * Tables * Streaming tables * Managed Iceberg tables * Foreign tables * Foreign Iceberg tables * Foreign Delta tables * Views * Materialized views | * Be the share owner. * Have the `USE CATALOG` and `USE SCHEMA` privilege on the catalog and schema that contain the data asset. * Have the `SELECT` privilege on the data asset. You must keep that privilege in order for the asset to continue to be shared. If you lose it, the recipient cannot access the asset through the share. |
