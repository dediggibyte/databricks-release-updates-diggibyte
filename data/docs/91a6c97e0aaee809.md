...
If the provider is using open sharing (which provides you with a credential file) but you have a Unity Catalog-enabled workspace, you can import the provider and read the shared data in Databricks. See [Import a provider and read shared data in Databricks](/aws/en/opensharing/read-data-open-databricks). If you don't have a Unity Catalog-enabled workspace, see [Read data shared with bearer tokens](/aws/en/opensharing/read-data-open).
...
**Permissions required to mount share to an existing catalog**: A user must have the `USE PROVIDER` privilege or have ownership of the provider object, and must also either own the existing shared catalog or have the `MANAGE` privilege on the existing shared catalog.
...
### Read data in a shared foreign Delta table

Preview

This feature is in [Public Preview](/aws/en/release-notes/release-types).

You can read data in a shared foreign Delta table using any of the tools available to you as a Databricks user: Catalog Explorer, notebooks, SQL queries, the Databricks CLI, and Databricks REST APIs. In Catalog Explorer, a shared foreign Delta table displays with a table type of **Foreign** and a data source format of **Delta**.

In Databricks-to-Databricks sharing, foreign Delta tables are always shared with full history and without a partition filter, so you always read the source Delta files directly using [cloud tokens](/aws/en/opensharing/create-share#cloud-token-eligibility).

**Requirements**:

* You must have the `SELECT` privilege on the shared foreign Delta table.
* You must use Databricks Runtime 16.4 LTS or above.

### Read data in a shared volume

You can read data in a shared volume using any of the tools available to you as a Databricks user: Catalog Explorer, notebooks, SQL queries, the Databricks CLI, and Databricks REST APIs. You must have the `READ VO