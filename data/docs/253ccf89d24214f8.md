...
You can add sample SQL queries, Unity Catalog functions, and plain-text instructions to help generate accurate responses. Click **Configure**. Use the **Examples** tab to manage example queries, filters, measures, fields, and joins. You can search for examples by keyword or example type.
...
Use the **Examples** tab to add the following:
...
1. Click **Configure** > **Examples** to access the list of example queries.
...
SQL functions are useful when a question involves complex logic that cannot be captured with a static or parameterized query. They are stored in Unity Catalog and can be called by Genie using user-supplied parameters. Genie supports both scalar functions, which return a single value, and table-valued functions, which return a table of rows. Genie cannot view or modify the SQL used in the function, making this approach well-suited for logic that should not be surfaced or changed. For guidance on registering a function in Unity Catalog, see [Create a SQL table function](/aws/en/sql/language-manual/sql-ref-syntax-ddl-create-sql-function#create-a-sql-table-function) and [SQL and Python user-defined functions (UDFs) in Unity Catalog](/aws/en/udf/unity-catalog).

When a SQL function is attached to an agent, you can view the function's description alongside it.
...
To access table metadata, click **Configure > Sources** in your Genie Agent. Then click a table name to view its metadata and columns.
...
If the data uses state abbreviations (such as `FL`), and Genie cannot access the values for that column, Genie might generate SQL that includes `ILIKE '%Florida%'`, which returns no